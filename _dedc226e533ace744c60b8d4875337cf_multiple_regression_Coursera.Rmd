...
ggplot(data = evals, aes(x=score)) + geom_histogram(binwidth = 0.1) + ggtitle("Score distribution")  
summary(evals$score)
sum(evals$score < 3)
```
...
ggplot(data = evals,aes(x = age, y = bty_avg)) + geom_point() + xlab("age") +ylab("beauty average")
...
#check normal distributited residuals
hist(m_bty_gen$residuals)
#check residuals constant variability 
plot(abs(m_bty_gen$residuals)~m_bty$fitted.values)
#check residuals independence
plot(m_bty_gen$residuals)
```

**(1)Linearity**
Both the "gender" variable and the "bty_avg"  variable have a linear relationship with the response variable - "score": In the box plot the median and variability of residuals are similar for male and female in terms of scores. In the residuals plot, as "bty_avg" increases, residuals fluctuate around 0.
**(2)Normal distribution**
From the third graph we can see that the distribution of residuals is left skewed, and not centered around 0, hence not normally distributed.
**(3)Constant variability**
Graph 4 show that yes the risiduals are somewhat constant. 
**(4)Residuals independence**
The residuals are randomly scattered so yes this condition is met.
...
summary(m_bty_gen)
```
**Yes "bty_avg" is still a significant predictor of "score". The addition of "gender" increased the parameter estimate for "bty_avg", as well as the adjusted R squared, which represents the predictive power of this model.**
...
**R generated two indicators called "ranktenure track" and "ranktenured" to represent two of the original levels: tenured and tenured track, both of which takes the value $1$, and the other catogory "teaching", takes the value$0$, for it comes first alphabetically thus it is the reference level.**

8. What is the correct order of the three levels of rank if we were to order them from lowest predicted course evaluation score to highest predicted course evaluation score? 

Tenured  --  tenure track -- teaching.

```{r}
ggplot(data = evals,aes(x = bty_avg, y = score, group = rank, color = rank)) + geom_point(size = 0.8,fill = "white") + stat_smooth(method = "lm", se = FALSE)
...
```{r p-val-select}
# type your code for the Exercise here, and Knit
m_full_adj1 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_full_adj1)
```

**Yes. Dropping this insignificant variable has resulted in lowering the p-value for most variavles, making them more significant predictors.**
...
m4<- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_level + cls_profs+ cls_credits + bty_avg + pic_outfit + pic_color, data = evals)
summary(m4)$adj.r.squared

m5<- lm(score ~ ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs+ cls_credits + bty_avg + pic_outfit + pic_color, data = evals)
...
```{r}
m3_5_5<- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
...
**There will not be an impact on any of these conditions since courses are independent of each other, no new collinearity added.**
...
**Based on the final model, a teacher who is teaching, not minority, male, formerly received education in non-English schools, with a high percent of students who completed the evaluation, with a relatively young age and high beauty average score, and usually dreassed informally and providing a black and white photo for his profile will be most likely to be associated with a high evaluation score.**
...
**I'm afraid not. The sample size is too small to be generalizable.**
